...
int CCircleDetect::identifySegment(SSegment* inner,CRawImage* image)
{
	...
	float m0 = inner->m0*1.0;
	float m1 = inner->m1*1.0;
	//calculate appropriate positions
	for (int a = 0;a<ID_SAMPLES;a++){
		x[a] = inner->x+(m0*cos((float)a/ID_SAMPLES*2*M_PI)*inner->v0+m1*sin((float)a/ID_SAMPLES*2*M_PI)*inner->v1)*2.0;
		y[a] = inner->y+(m0*cos((float)a/ID_SAMPLES*2*M_PI)*inner->v1-m1*sin((float)a/ID_SAMPLES*2*M_PI)*inner->v0)*2.0;
	}
...
//	if (debug){
		...
		printf("\n");
		for (int a = 0;a<ID_SAMPLES;a++)printf("%i ",a%segmentWidth == maxIndex%segmentWidth);
		printf("\n");
//	}
...
SSegment CCircleDetect::findSegment(CRawImage* image, SSegment init)
{
	...
	if (outer.valid && identify){
		inner.x = outer.x; 
		inner.y = outer.y; 
		inner.m0 = 0.60*outer.m0; 
		inner.m1 = 0.60*outer.m1;
		inner.v0 = outer.v0;
		inner.v1 = outer.v1;
